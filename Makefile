...
# 
FILE_DA_CONSEGNARE=DATA/chatty.conf1 DATA/chatty.conf2 script_bash \
				   Build_ename.sh chatterbox17.pdf chatty.c		   \
				   client.c config.h connections.c connections.h   \
				   Doxyfile ename.c.inc error_functions.c 		   \
				   error_functions.h get_num.c get_num.h		   \
				   icl_hash.c icl_hash.h listener.c Makefile 	   \
				   message.h mkpath.c mkpath.h ops.h parser.c 	   \
				   parser.h queue.c queue.h rdwrn.c rdwrn.h 	   \
				   README.doxygen README.txt relazione_518111.pdf  \
				   stats.h testconf.sh testcustom.sh testfile.sh   \
				   testgroups.sh testleaks.sh teststress.sh 	   \
				   utlist.h workers.h							   \
...
DMY_HMS	=	$(shell date +%d-%m-%Y_%H.%M.%S)
FNAMES 	=	$(FILE_DA_CONSEGNARE)
BCK_DIR	=	./

backup:	
	tar czf $(BCK_DIR)$(DMY_HMS).tar.gz $(FNAMES)

chatty: chatty.o libchatty.a
	$(CC) $(CFLAGS) $(INCLUDES) $(OPTFLAGS) $(LDFLAGS) -o $@ $^ $(LIBS)

client: client.o connections.o mkpath.o rdwrn.o
	$(CC) $(CFLAGS) $(INCLUDES) $(OPTFLAGS) $(LDFLAGS) -o $@ $^ $(LIBS)
...
testcustom:
	...
	./testcustom.sh $(UNIX_PATH)
	killall -QUIT -w chatty
	@echo "********** TestCustom superato!"
